...
library(tidyverse) 
library(dplyr)
# tidyverse, tidyr ve dplyr gibi birçok kullanışlı paketi içerir

# Veriyi yükle
users <- read.csv("C:/Users/Acer/Desktop/donemproje1_fecriati/users.csv", sep = ";", stringsAsFactors = FALSE)
...
users <- users %>%
  mutate(across(where(is.character), ~ifelse(. == "", NA, .)))  # Boş karakter dizilerini NA ile değiştir

# Eksik verileri kontrol et ve doldur
users <- users %>%
  mutate(across(where(is.numeric), ~ifelse(is.na(.), mean(., na.rm = TRUE), .)))

users <- select(users, -c(latitude, longitude, X))

# Sonuçları kontrol et
summary(users)

write.csv(users, "C:/Users/Acer/Desktop/donemproje1_fecriati/user_yeni.csv", row.names = FALSE)
```
...
# Veriyi yükle
orders <- read.csv("C:/Users/Acer/Desktop/donemproje1_fecriati/orders.csv", sep = ";", stringsAsFactors = FALSE)
...
orders <- select(orders, -c(returned_at,shipped_at,delivered_at))

# Sonuçları kontrol et
summary(orders)

write.csv(orders, "C:/Users/Acer/Desktop/donemproje1_fecriati/order_yeni.csv", row.names = FALSE)
```
...
# Veriyi yükle
order_items <- read.csv("C:/Users/Acer/Desktop/donemproje1_fecriati/order_items.csv", sep = ";", stringsAsFactors = FALSE)
...
order_items <- select(order_items, -c(returned_at,shipped_at,delivered_at))

# Normal dağılım ile rastgele fiyatlar oluşturma
set.seed(123)  # Rastgelelik için çekirdek ayarı
random_prices <- rnorm(n = nrow(order_items), mean = 200, sd = 50)

# Negatif fiyatları kaldırma
random_prices <- ifelse(random_prices < 0, 0, random_prices)

# sale_price sütununu güncelleme
order_items$sale_price <- random_prices

# Sonuçları kontrol et
summary(order_items)

write.csv(order_items, "C:/Users/Acer/Desktop/donemproje1_fecriati/order_items_yeni.csv", row.names = FALSE)
```
